...
let Component = false
let ComponentDataSources = false
let ComponentEvents = false;
...
(function () {
  ...
  const loadScript = function (name, path) {
  ...
  const stillWaitingForRequirements = function () {
    let stillWaiting = false
    for (const key in requiredScripts) {
      if (!window[key]) {
        stillWaiting = true
        break
      }
  ...
  const runDeferredScript = function (callback) {
    (function F () {
  ...
  for (const key in requiredScripts) {
    if (!window['Loading' + key] || typeof window[key] === 'undefined') {
      const path = requiredScripts[key]
      loadScript(key, path)
    }
  }
  ...
  Component = (function () {
    ...
    Class.configure = function (elementName) {
      ...
      let ComponentClass = Class.registeredComponents[elementName] || false
      if (ComponentClass) {
    ...
    Class.findByElement = function (element) {
      ...
      const webComponentId = element.getAttribute('webcomponentid')
    ...
    Class.registerComponentClass = function (elementName, template) {
      const ComponentClass = Component.create(elementName, template)
    ...
    Class.create = function (elementName, templateElement) {
      const ComponentClass = function (element) {
      ...
      ComponentClass.prototype.construct = function (element) {
        ...
        const preferredTemplate = $(this.templateKeyPreferred)[0]
      ...
      ComponentClass.prototype.preRenderStep = function (fn) {
        ...
        const steps = ComponentClass.prototype.preRenderSteps
        if (fn) {
      ...
      ComponentClass.prototype.requestDataFromSource = function () {
        if (typeof ComponentDataSources !== 'undefined' && ComponentDataSources.adapters) {
          const list = ComponentDataSources.adapters
      ...
      ComponentClass.prototype.updateDataSourceStatus = function (message) {
        const existsInPage = Class.existsInPage(this.element)
        if (existsInPage) {
      ...
      ComponentClass.prototype.render = function (reason) {
        ...
        const expandedTemplate = Component.expandTemplate(this, $(this.template).html(), elementName)
      ...
      ComponentClass.apply = function (fn) {
        ...
        const instances = ComponentClass.registeredInstances
        for (id in instances) {
    ...
    Class.expandTemplate = function (data, template, context) {
      if (template) {
        ...
        try {
          ...
          const templateDebug = template.replace(/</g, '&lt;').replace(/>/g, '&gt;')
    ...
    Class.tab = function () {
      let tab = ''
      for (let i = 0; i < Class.scanLevel; i++) { tab += '  ' }
    ...
    Class.log = function (message, level) {
      ...
      if (Class.logLevelValue >= level) {
        const type = LogLevel.types[level] || 'unknown'
    ...
    Class.registerTemplates = function (rootElement) {
      ...
      const templates = $('template[for], template[tagname]', rootElement).each(function () {
        const template = this
        const elementName = $(this).attr('for') || $(this).attr('tagname')
    ...
    Class.scanForComponents = function (rootElement, type, reRender) {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (!element.attributes.rendered) {
          const ComponentClass = Class.registeredComponents[elementName] || false
          if (ComponentClass) {
